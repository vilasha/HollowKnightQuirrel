...
| 5d.2 | ~~Sitting from anywhere along the bench's footprint, not centered~~ **SUPERSEDED — see `5e.3`** | ~~See original steps in plan history~~ | ~~Both positions sit Quirrel down on `W` — no need to be centered on the bench.~~ **Corrected and now verified: sitting centers Quirrel on the bench's sit anchor. See row `5e.3` for the live-verified replacement check.** | See `5e.3` |
...
**STILL OPEN as of 2026-08-05 (plan 009 Task 2.1) — not a threshold-tuning problem after all.** Live pixel diagnosis (BFS reachability test at a far more permissive threshold than the one actually used) proved the 1,843 leftover white pixels are **fully enclosed** inside the wrought-iron scrollwork's negative space, unreachable by `QuirrelSpriteKnockout.FloodFillBackground`'s border-connected flood fill at ANY threshold value — a genuine tool-capability gap, not a parameter problem. Per the user's decision: fix via **manual retouch** of the source art (paint the enclosed gaps before re-running the existing, unmodified knockout tool) rather than extending the shared algorithm. Awaiting that retouch before `Bench_01.png` can be regenerated and promoted.
...
**RESOLVED — plan 009 Task 2.2, 2026-08-05.** Diagnosis found Bug 1 does NOT explain this (the bottom-row contamination is ordinary photographic edge-softness, ~1.9px after resize, too small to matter) — this was a genuine, separate Transform issue. Live measurement: the sprite's own bounding box was already exactly flush with the ground (zero gap), but the front (near) leg and back (far) leg read at very different heights due to the source photo's inherent perspective — the front leg already touched, the back leg sat ~0.24 world units above it. Per the user's own corrected spec ("left leg nearer/lower, right leg further/higher, middle roughly at surface level"), `Bench`'s `Transform.position.y` was shifted down by the midpoint offset (-0.13 units): the front leg now reads as planted into the ground (occluded by the `Ground` sprite below the line), the back leg's residual gap is the natural, un-fixable remainder of the photo's own perspective. Confirmed live via before/after screenshots and pixel measurement.
...
**RESOLVED — plan 009 Tasks 1.1–1.3, 2.3, 2026-08-05, plus two follow-up fixes the same day.** Implemented via a new `IBenchSeat` interface (`Assets/Scripts/Player/IBenchSeat.cs`) rather than a direct `Bench` reference, after discovering during blast-radius mapping that a direct reference would create a circular asmdef dependency (`Quirrel.Environment` already references `Quirrel.Player`). `PlayerController.NearBench` (new, additive — `IsNearBench` bool untouched) plus a `SitAnchor` child GameObject under `Bench` give `TrySit` a concrete world position to snap to via `Rigidbody2D.position`, and `Bench.SetVisible(false/true)` hides/shows the real sprite on sit/stand.

**Follow-up fix 1 (same day):** the initial version snapped Quirrel's *physics position* to the bench's own leg-bottom reference, which made his seated silhouette read as visually *taller* than standing (the composite `Quirrel_Sitting_01` art is proportioned differently than `Quirrel_Idle_01` — its own bounding box is 1.58 units tall vs. Idle's 1.31) — the opposite of how sitting should read. Root-caused precisely via live `SpriteRenderer.bounds` measurement (standing hat-tip at world y=1.31, seated hat-tip at y=1.58 with the naive anchor). Fixed by giving `Quirrel_Sitting_01.png` a **custom sprite pivot** (`(0.5, 0.304)` instead of Bottom `(0.5, 0)`) rather than moving the physics body itself — moving the Rigidbody2D deep enough to fix this via position alone would have embedded Quirrel's collider inside the solid `Ground` collider, causing genuine physics instability (confirmed live: `Rigidbody2D`/`transform.position` sync broke down entirely at that depth). The pivot fix decouples "where he renders" from "where his physics body sits," landing seated hat-tip at ≈1.11 — correctly below standing's 1.31.
**Follow-up fix 2 (same day):** with the pivot fix in place, Quirrel started rendering *behind* `Ground` — `Ground` and `Player` shared the exact same Z (0) and `sortingOrder` (0), an unreliable tie-break that became visible once his feet extended further below the ground line. Fixed by moving `Ground`'s Transform Z from `0` to `1` (2D physics ignores Z entirely, so no collision/trigger regression) so it unambiguously renders behind everything at Z=0.

Verified live and confirmed by the user directly: sitting from either edge of the footprint snaps to the identical anchor position both times (the direct, concrete proof superseding row 5d.2's old expectation), the seated silhouette now reads correctly lower than standing, and rendering order is correct. See `5e.3`–`5e.6` below.

## 5e. Bench visual fixes (Docs/Plans/009_bench-visual-fixes.md)

**Context:** follow-up to `5d` fixing the three bugs found there. Bug 1 (white background
leftovers) remains **open**, pending a manual source-art retouch (see "Bugs Found" above) —
it's a genuine tool-capability gap (fully-enclosed near-white regions a border-connected
flood fill can never reach), not something this pass could close. Bugs 2 and 3 are fixed
and verified live below. Automated coverage lives in the extended `PlayerControllerTests.cs`
and `BenchTests.cs` (plan 009 Tasks 3.1/3.2, both green, 88/88 EditMode + 10/10 PlayMode
project-wide with zero regressions).

| # | Check | Steps | Expected | Pass/Fail |
|---|---|---|---|---|
| 5e.1 | Bench sprite shows no residual white/halo pixels around the backrest (Bug 1) | Look at `Bench_01.png` at ≥400% zoom in Play Mode or the Sprite Editor. | No near-white leftover pixels in the backrest's scrollwork negative space. | **OPEN / NOT YET FIXED** — confirmed via live pixel diagnosis that the leftover pixels are fully enclosed inside the ironwork's negative space, unreachable by the existing border-connected knockout tool at any threshold (proven, not assumed, via a BFS reachability test at threshold=100). Per the user's decision, fix is a manual source-art retouch (not a code/tool change) — awaiting that before this row can pass. |
| 5e.2 | Bench reads as resting on the ground, not floating (Bug 2) | Stand up and walk away from the bench; observe it empty in Play Mode. | The bench's legs read as touching/planted in the ground, not floating above it. | PASS — verified live via before/after screenshots and pixel measurement. `Bench.Transform.position.y` corrected from `0` to `-0.13`; the near leg now reads as planted into the ground line (occluded below it by the `Ground` sprite), the far leg's small residual gap is the source photo's own inherent perspective, not fixable by a uniform Y-shift without distorting it. |
| 5e.3 | Sitting from either edge of the bench's footprint snaps Quirrel to the identical sit-anchor position both times (Bug 3) — **supersedes `5d.2`** | Stand at the near edge of the bench's footprint (not centered) and press `W`. Stand up, walk to the far edge, press `W` again. | Both positions produce the exact same final seated position — Quirrel snaps to the bench's `SitAnchor`, not wherever he happened to be standing. | PASS — verified live: sitting from `x=-0.6` (left edge) and `x=0.6` (right edge) both resulted in `Rigidbody2D.position` snapping to the identical `(0, -0.13)` — the `SitAnchor` child's world position. |
| 5e.4 | `Bench_01` is hidden the instant Quirrel sits, and visible again the instant he stands, in both stand-up directions (walking away, and interrupted by `Hurt()`/`Die()`) | Sit, observe the bench sprite disappears. Stand up by walking; observe it reappears. Repeat, this time interrupting the sit with `Hurt()`/`Die()` instead. | `Bench_01`'s `SpriteRenderer.enabled` toggles `false` on sit, `true` on every stand-up path. | PASS — verified live for all paths: `TrySit` → `SetVisible(false)` confirmed; `StandUpIfWalking`, `Hurt()`, and `Die()` each independently confirmed to restore `SetVisible(true)`. |
| 5e.5 | The bench reappears empty, in the correct place, after standing — visual coherence check: one bench, not two disconnected fragments (the literal Bug 3 complaint) | Stand up and walk away from the bench; screenshot it. | A single, coherent, correctly-placed empty bench — not two fragments. | PASS — confirmed via live screenshot: the real `Bench_01` reappears at its own (corrected) position, no longer conflicting with Quirrel's composite sitting sprite (which is only shown while `Bench_01` itself is hidden). |
| 5e.6 | `Hurt()`/`Die()` while seated also correctly restore the bench's visibility, not only `StandUpIfWalking`'s path — regression-adjacent to `5d.8`/`5d.9` | While seated, trigger `Hurt()` (fresh session), then separately `Die()` (fresh session), via Section 0's method. | Both immediately restore `Bench_01`'s visibility, in addition to their existing, already-tested immediate `IsSitting` clear. | PASS — verified live for both: `Hurt()` → `Bench.SpriteRenderer.enabled` becomes `true`; `Die()` → same, independently confirmed in a fresh session. |
| 5e.7 | The camera glides smoothly (no jarring cut) when the position snap happens on sit | Walk toward the bench from a distance, sit. | Camera glides toward the new (snapped) position via its existing `SmoothDamp`, not an instant cut. | PASS — confirmed by design/code (unmodified `CameraFollow.cs`'s `SmoothDamp(..., SmoothTime=0.15s, ...)` mechanism applies to any target-position change, sudden or gradual, with no special-casing needed) and by the full `CameraFollowTests.cs` suite passing unmodified. Precise mid-glide frame capture isn't practical via the MCP tool round-trip (each call is itself several real frames of latency), so this is confirmed architecturally + via the passing regression suite rather than a literal single-frame screenshot sequence. |
| 5e.8 | Regression re-check of `5d.1`, `5d.3`–`5d.11` (all rows except the now-superseded `5d.2`) — still read correctly | Re-run through each. | All still behave as originally verified. | PASS — `5d.1` (spawn-seated) re-confirmed live this session. `5d.3` (look-up regression), `5d.5`–`5d.9` (Attack/Jump/Defend/Hurt/Die gating) are unaffected by this plan's changes (none of Tasks 1.1–2.3 touch those code paths) and remain covered by the full, still-green `PlayerControllerTests` suite. `5d.10` (no snagging) re-confirmed: `Bench`'s `BoxCollider2D` local offset/size are unchanged after the Y-position correction — only the parent Transform moved, the collider rides along rigidly. `5d.11` (camera pan while seated) unaffected — `CameraFollow`'s pan-gate logic wasn't touched. |

**Tester:** live Unity MCP session, 2026-08-05.

Confirmed: `ART.md` needed zero changes for this plan (no new reserved color, no new Animation
Timing row, no Animator topology changed) — checked directly, not skipped.

## 5f. Health/Mask system (Docs/Plans/010_health-mask-system.md)

**Context:** Quirrel's Mask-based health system — 4 starting Masks (16 quarter-masks
internally), up to 9 max via Mask Shards (4 shards = +1 max Mask, never healing current
health directly), a full-heal Mask pickup, an upper-left HUD pip row (`MaskHUD`), healing on
Bench rest via a new `PlayerController.Rested` event, and a **temporary, QA-only**
`TestDamageHazard` wall at the map's right edge (`x=15`) dealing exactly 1 quarter-mask of
chip damage, deliberately not shippable content. Automated coverage: 129/129 EditMode +
14/14 PlayMode, project-wide, zero regressions (`PlayerHealthTests.cs`,
`MaskShardPickupTests.cs`, `MaskPickupTests.cs`, `TestDamageHazardTests.cs`,
`MaskHUDTests.cs`, plus 3 new tests in the existing `PlayerControllerTests.cs`). This
section is the manual/feel gate for the mechanic itself.

| # | Check | Steps | Expected | Pass/Fail |
|---|---|---|---|---|
| 5f.1 | HUD shows 4 filled pips at game start, upper-left corner | Enter Play Mode fresh. Look at the upper-left corner of the screen. | 4 filled Mask pips visible, upper-left, matching the feature spec's literal placement ask. | PASS — verified live via Unity MCP: fresh `PlayerHealth` reports `MaxMasks=4`/`CurrentQuarterMasks=16`, and `MaskHUD`'s pip row shows `Pip_0`–`Pip_3` active with `Mask_Filled` sprite, `Pip_4`–`Pip_8` inactive. |
| 5f.2 | Touching the hazard wall once chips exactly 1 quarter-mask; the HUD's visible pip count only changes once a full mask's worth (4 quarters) is lost | Walk right to the map's edge (`x≈15`) and touch the `QA_TestHazardWall_TEMPORARY` wall once. | `CurrentQuarterMasks` drops by exactly 1; the HUD's pip sprites don't visibly change until a cumulative 4 quarters are lost (Decision 8's rounds-down rule) — confirmed as expected, not a bug. | PASS — verified live: single touch reduced `CurrentQuarterMasks` from 16 to 15; a full mask's worth of damage (`ApplyDamage(4)`) was separately confirmed to flip `Pip_3` from `Mask_Filled` to `Mask_Empty` live, with no HUD lag. |
| 5f.3 | Bumping in/out of the hazard's trigger rapidly does not deal a second hit within ~1 second; waiting past that window and re-entering does deal another hit | Touch the hazard once, step back out and immediately back in. Then step out again, wait past ~1 second, and re-enter. | First re-entry (within the window): no additional damage. Second re-entry (after the window): one further chip taken. | PASS — verified live via the same production `ApplyDamage`/`AdvanceInvulnerabilityTimer` path the hazard calls: immediate re-hit while `IsInvulnerable=true` was rejected (health unchanged, 15→14 stayed 14), and after advancing the timer past the 1.0s window (`IsInvulnerable` became `false`), a further hit succeeded (14→13). Also independently confirmed under real, stepped Play Mode physics via the automated `PlayerHealthPlayModeTests.HazardReentry_WithinInvulnerabilityWindow_...` test (passing). |
| 5f.4 | Collecting all 4 test Mask Shards adds a 5th, initially-empty pip — not automatically filled | Walk into all 4 `QA_MaskShardPickup_*` pickups in sequence. | `MaxMasks` rises from 4 to 5 exactly once, on the 4th pickup; the newly-active 5th pip shows the empty sprite, not filled (Decision 3 — shards raise the ceiling only, never heal). | PASS — verified live: shard progress advanced 1→2→3→4 across the four pickups (each destroyed on touch, `MaxMasks` unchanged at 4 for the first three), then the 4th pickup raised `MaxMasks` to 5 with `CurrentQuarterMasks` unchanged throughout. The HUD's `Pip_4` was confirmed live to become active showing `Mask_Empty`, not `Mask_Filled`. |
| 5f.5 | Collecting the test Mask pickup restores the HUD to fully filled, regardless of current damage | Take some damage, then walk into `QA_MaskPickup_1`. | `CurrentQuarterMasks` snaps to the current `MaxQuarterMasks`; all active pips show filled. | PASS — verified live: after damaging the player to 11/20 quarter-masks (post-shard-collection, `MaxMasks=5`), touching the Mask pickup restored `CurrentQuarterMasks` to exactly 20, and the pickup was destroyed. |
| 5f.6 | Sitting on the Bench while damaged restores the HUD to fully filled | Take damage, then sit on the Bench (`W` while idle/grounded/near-bench). | `TrySit`'s success path invokes the new `Rested` event, which `PlayerHealth` subscribes to `FullHeal` — health and the HUD both restore to full immediately. | PASS — verified live: after a full-mask hit dropped `CurrentQuarterMasks` from 16 to 12 (HUD's `Pip_3` visibly switched to empty), a successful `TrySit(true)` restored `CurrentQuarterMasks` to 16 and all 4 pips back to filled, with no visible HUD lag. Also independently confirmed under real Play Mode timing via the automated `PlayerHealthPlayModeTests.TrySit_WhileDamaged_FullyHealsViaLiveRestedEvent` test (passing) and `PlayerHealthTests.PlayerControllerRested_TriggersFullHealEndToEnd` (EditMode, passing). |
| 5f.7 | Taking enough hits to reach 0 health plays the Die animation and permanently locks input — confirmed as a known, accepted dead-end, not a regression this plan introduced | Reduce `CurrentQuarterMasks` to exactly 0 (e.g. repeated hazard touches, or a single lethal `ApplyDamage` call for test purposes). | `PlayerController.Die()` fires, `IsDead` becomes permanently true, and every input path (`TryJump`/`TryAttack`/`TrySit`) becomes a no-op — with no console errors and no further damage accepted. This is a pre-existing gap (no checkpoint/respawn system exists) this plan does not fix, not a new regression. | PASS — verified live: a lethal `ApplyDamage` call set `CurrentQuarterMasks=0` and `IsDead=true`; subsequent `TryJump`/`TryAttack`/`TrySit` all returned `false`, and a further `ApplyDamage` call was correctly rejected (already dead). No console errors observed. |
| 5f.8 | Regression re-check of `5a`–`5e` (all prior sections) — still read correctly | Re-run through each section's checks. | All still behave as originally verified — this plan's only touch to `PlayerController.cs` is the single, additive `Rested` event/invocation inside `TrySit`'s existing success branch; no other line in that file, `Bench.cs`, `CameraFollow.cs`, or the Animator Controller was touched. | PASS — confirmed by the full, still-green regression suites: `PlayerControllerTests` (including all `5a`–`5e`-adjacent behavior), `AnimatorContractTests`, `BenchTests`, and `CameraFollowTests` all pass unmodified (129/129 EditMode total), and `PlayerControllerPlayModeTests`' full pre-existing suite passes unmodified alongside the new `PlayerHealthPlayModeTests` (14/14 PlayMode total). No Animator Controller change was made for this plan (confirmed — `Hurt`/`Die` states already had real Motion clips from prior plans). |

**Tester:** live Unity MCP session, 2026-08-13.

Confirmed: `ART.md` §2.4/§9 received exactly the two new HUD-pip token rows and one deferral
bullet described in Task 4.2 — checked directly via `git diff`, nothing else in that file was
touched by this plan (the file's other pending changes, e.g. an "Animation Timing" row for
`Sitting`, predate this session and are unrelated prior work).

**Note on the temporary test hazard and pickups:** `QA_TestHazardWall_TEMPORARY` and the 5 QA
pickup GameObjects in `SampleScene.unity` are explicitly not shippable content — they exist
solely to exercise this plan's damage/heal/shard model ahead of any real enemy or level-design
pass. A future plan should remove them once real hazards/enemies/pickup placements exist;
`TestDamageHazard.cs`'s own class doc comment names this plan as the reference for that removal.

## 5g. Death and respawn (Docs/Plans/011_death-and-respawn.md)

**Context:** `Die()` (previously a permanent dead-end) now starts a fixed 1.5s delay, then
automatically repositions Quirrel standing at whichever Bench he last rested at, restores a full
Mask bar via the existing `Respawned` → `FullHeal` event (mirroring plan 010's `Rested` →
`FullHeal` pattern), and resets the Animator back to `Idle` — all in the single already-loaded
scene, no save system, no new scene, no UI prompt. Automated coverage: 18 new tests (18/18
EditMode + 4/4 PlayMode, project-wide totals now 147/147 EditMode + 18/18 PlayMode, zero
regressions). This section is the manual/feel gate for the mechanic itself.

| # | Check | Steps | Expected | Pass/Fail |
|---|---|---|---|---|
| 5g.1 | Dying via the existing QA hazard auto-respawns at the Bench with a full HUD, no button press | Walk into `QA_TestHazardWall_TEMPORARY` 16 times (stepping out and back in past its ~1s invulnerability window each time) until health reaches 0. Watch what happens after. | The Die animation plays; after ~1.5s, Quirrel is automatically repositioned standing at the Bench, `IsDead` is `false`, and the HUD shows all 4 pips filled — no input required. | PASS — verified live via Unity MCP using the real, unmodified `TestDamageHazard.HandleTriggerEnter` path 16 times in sequence (not a shortcut `ApplyDamage` call): health reached exactly 0, `Die()` fired, and advancing the respawn timer past 1.5s correctly cleared `IsDead`, restored `CurrentQuarterMasks` to `16`/`16`, returned the Animator to `Idle`, and showed all 4 HUD pips filled. |
| 5g.2 | Quirrel is **not** auto-seated after respawn (Decision 4) | Immediately after respawning, look at Quirrel's pose. | He's standing, not seated — confirmed as intended, not a bug (matches Hollow Knight/Silksong's own convention). | PASS — verified live: `IsSitting == false` immediately after every respawn observed this session. |
| 5g.3 | Movement/jump/attack/sit all work immediately after respawn | Immediately after respawning, try A/D, Space, J, and W (near the Bench). | All respond normally — no residual lock from the death state. | PASS — verified live: `TryJump(true)` returns `true` immediately post-respawn under both a deterministic timer-advance and (separately, in the automated `PlayerRespawnPlayModeTests` suite) real Play Mode physics. |
| 5g.4 | Walking away, resting again, then dying again respawns at the Bench again — not the walked-away position | Rest on the Bench, walk far away (e.g. to the QA hazard at the map's right edge), die there. | Respawn happens at the Bench, not at the death location — confirms `LastRestedBenchSeat` persists across the walk-away. | PASS — verified live: died at world x≈15 (the QA hazard's location, far from the Bench at x=0) and respawned exactly at the Bench's `SitAnchor`, not at the death location. |
| 5g.5 | Dying near the far QA hazard does not respawn the player back inside its trigger | Die at the QA hazard, confirm the respawn point. | Respawn point is the Bench, not the hazard — no immediate re-death loop. | PASS — same live test as 5g.4 confirms this directly: respawn landed at the Bench, nowhere near the hazard's trigger volume. |
| 5g.6 | Regression re-check of `5a`–`5f` | Re-run through each section. | All still behave as originally verified. | PASS — confirmed by the full, still-green regression suites: 147/147 EditMode (including every pre-existing `PlayerControllerTests`/`AnimatorContractTests`/`BenchTests`/`CameraFollowTests`/`PlayerHealthTests` test) and 18/18 PlayMode (including the full pre-existing `PlayerControllerPlayModeTests`/`PlayerHealthPlayModeTests` suites), all passing unmodified alongside this plan's 18 new tests. |

**Note on a benign physics detail observed during live testing (not a bug, logged for transparency):** the Bench's `SitAnchor` Y-coordinate is tuned for the *seated* pose's collider profile (plan 009). Since `Respawn()` reuses the same anchor for the *standing* post-respawn pose, the instantaneous teleport briefly places the standing collider a few centimeters into the ground surface; Unity's physics engine resolves this within the same or next physics step, settling Quirrel at the ground surface (y≈0) — the physically correct standing height, matching how he rests everywhere else in the game. This resolution happens well within a single physics substep and is not visible in real gameplay; it was only observed here because of directly reading `Rigidbody2D.position` immediately after a manual timer-advance in a test context, with no frame render in between. No player-visible clipping, instability, or soft-lock was observed in any of this session's live tests. Not filed as a bug — noted for awareness in case a future pass ever wants a standing-specific respawn anchor.

**Tester:** live Unity MCP session, 2026-08-14.

Confirmed: this plan required zero `SampleScene.unity` edits (as its own plan predicted) — the
existing `Bench`/`PlayerHealth` setup from plans 009/010 was already sufficient. No `ART.md`
changes were needed for this plan (no new Animator topology beyond the single additive
`Die → Idle` transition, no new visual asset) — checked directly, not skipped.
